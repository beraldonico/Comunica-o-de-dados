...
for k = 1 : length(harmonicos)
    ...
    for i = steps_per_bit_receiver/4 : steps_per_bit_receiver : length(sinal_filtrado(k,:))
        if i == steps_per_bit_receiver/4
            if sinal_filtrado(k,i) < 1
        ...
        elseif abs(sinal_filtrado(k,i) - sinal_filtrado(k,i - steps_per_bit_receiver/2)) > 1.5
            for j = 1 : steps_per_bit_receiver
                signal_out(k, w++) = 0;
            endfor
        else
